...
export default function UserDataModelUpdateForm(props) {
  const {
    username: usernameProp,
    userDataModel: userDataModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userDataModelRecord
      ? { ...initialValues, ...userDataModelRecord }
      : initialValues;
    setUsername(cleanValues.username);
    setStreak(cleanValues.streak);
    setLevel(cleanValues.level);
    setHasOnboarded(cleanValues.HasOnboarded);
    setErrors({});
  };
  const [userDataModelRecord, setUserDataModelRecord] = React.useState(
    userDataModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = usernameProp
        ? await DataStore.query(UserDataModel, usernameProp)
        : userDataModelModelProp;
      setUserDataModelRecord(record);
    };
    queryData();
  }, [usernameProp, userDataModelModelProp]);
  React.useEffect(resetStateValues, [userDataModelRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            UserDataModel.copyOf(userDataModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "UserDataModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Username"
      ...
      >
        <Button
          ...
          isDisabled={!(usernameProp || userDataModelModelProp)}
